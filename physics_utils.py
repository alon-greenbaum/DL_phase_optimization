...
# PyTorch-based NoiseLayer class
class NoiseLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        # All parameters must be provided in the config dictionary.
        # Ensure your config dictionary contains all expected keys.

        # Assign parameters directly as attributes.
        # Note: Without register_buffer, these values will not be automatically
        # moved to device with .to(device) and will not be saved/loaded with state_dict.
    ...
    def forward(self, ideal_image_volume):
        ...
        Args:
            ideal_image_volume (torch.Tensor): The ideal, noiseless image volume.
                                               This should represent the *incident photon counts*
                                               at each pixel after PSF application, or be directly
                                               proportional to it.
                                               Can be 2D (height x width) or 3D (depth x height x width).
        ...
        # Ensure ideal_image_volume is float64 for numerical stability, and on the correct device
        ...
        #    This step accounts for the quantum efficiency of the sensor.
        signal_electrons = ideal_image_volume * self.quantum_efficiency.to(ideal_image_volume.device)

        # 2. Add Dark Current Electrons.
        #    Dark current is also a random process, often modeled as Poisson.
        #    These are electrons generated thermally, even without incident light.
        #    torch.poisson expects the mean to be a tensor.
        #    torch.poisson uses random values from the CPU's default generator
        #    but moves the result to the input's device.
        dark_current_electrons = torch.poisson(self.dark_current_mean.to(ideal_image_volume.device).expand_as(ideal_image_volume))

        # 3. Calculate the total mean electrons accumulated per pixel before readout.
        #    Ensure the mean is non-negative before applying Photon Noise.
        total_mean_electrons = signal_electrons + dark_current_electrons
        total_mean_electrons = torch.clamp_min(total_mean_electrons, 0.0) # Ensure non-negative mean for Poisson

        # 4. Apply Photon Noise (Shot Noise).
        #    This is a fundamental noise source due to the discrete nature of photons.
        #    The actual number of electrons generated follows a Poisson distribution
        #    around the `total_mean_electrons`. The variance of Poisson noise is equal to its mean.
        noisy_electrons_poisson = torch.poisson(total_mean_electrons)

        # 5. Add Read Noise.
        #    This noise is primarily due to the camera's readout electronics and is
        #    independent of the signal level. It is typically modeled as Gaussian.
        #    Generate Gaussian noise on the same device as the input tensor.
        ...
        electrons_after_noise = noisy_electrons_poisson + read_noise

        # 7. Apply Camera Gain (Analog-to-Digital Conversion).
        #    Converts the electron count into Analog-to-Digital Units (ADU).
        noisy_image_adu = electrons_after_noise * self.camera_gain.to(ideal_image_volume.device)

        # 8. Clip values to the valid ADU range.
        #    Pixel values cannot be negative, and they cannot exceed the camera's maximum ADU.
        noisy_image_adu = torch.clamp(noisy_image_adu, min=0.0, max=self.camera_max_adu.to(ideal_image_volume.device))

        return noisy_image_adu.type(torch.float32)  # Convert back to float32 for consistency with input type
...
class PhysicalLayer(nn.Module):
    def __init__(self, config, initial_phase_mask = None):
    ...
    def against_lens(self):
        Ta = torch.exp(1j * self.phase_mask) # amplitude transmittance (in our case the slm reflectance)
        Ta = Ta[None, None, :] 
    ...
    def forward(self, phase_mask, xyz):
        self.phase_mask = nn.Parameter(phase_mask)
        Nbatch, Nemitters = xyz.shape[0], xyz.shape[1]
        
        if self.lens_approach == 'fresnel':
            mask_param = self.incident_gaussian * torch.exp(1j * self.phase_mask)
